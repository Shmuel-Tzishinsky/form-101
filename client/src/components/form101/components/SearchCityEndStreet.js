...
function SearchCityEndStreet({
      ...
      if (
        ...
        if (data.success) {
          let mapCiti = data.result.records
            .filter(
              (e) =>
                e['שם_ישוב'] === allData.employeeDetails.citi &&
                e['שם_ישוב'].replace(/^\s+|\s+$/g, '') !==
                  e['שם_רחוב'].replace(/^\s+|\s+$/g, '')
            )
            .map((e) => e['שם_רחוב']);
      ...
      if (!street) {
        ...
        if (data.success) {
          ...
          if (arrayData) {
            ...
            let newArray = [];
            s.forEach(
              (e) => (newArray = array_move(arrayData, arrayData.indexOf(e), 0))
            );
  ...
  const searchCiti = (e) => {
    ...
    const filterData = allOption.filter((item) =>
        ...
        <div>
          <label style={{ position: 'relative' }}>
            <input
              ...
              autoCorrect="off"
              defaultValue={
                street && showOption.length === 0
                  ? `אין רחובות ב${allData.employeeDetails.citi}`
                  : selectCiti
              }
                  ...
                  <div className="all-options">
                    {showOption.map((e, k) => (
                      <div
                        key={k}
                        onMouseDown={() =>
                          e !== 'לא נמצאו תוצאות' && handleCiti(e)
                        }
                      >
                        {e}
                      </div>
